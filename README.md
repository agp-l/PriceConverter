...
The app shows the age of its last downloaded rates and lets you refresh them. It reuses a saved rate for at least five minutes instead of fetching on every launch; when reopened, reconnected, or brought to the foreground, it fetches again only if the cached rates and last request are at least five minutes old. There is no continuous background polling. A manual refresh is available after one minute. The time of the last attempt is stored locally so repeated launches and failed requests do not immediately retry. These limits apply per browser; a public static app cannot guarantee the provider's shared IP limits across users. Its service worker caches the app files for offline opening after a successful visit; the last downloaded rates are kept in browser storage. Offline results can be stale. On a first visit without a connection or a saved rate, conversions that need a rate are unavailable. TradingView embeds are fetched from TradingView and are not part of the offline app cache.
