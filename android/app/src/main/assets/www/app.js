...
export class ConverterApp {
  ...
  updateInstallButton() {
    this.elements.install.hidden = this.win.location.hostname === 'appassets.androidplatform.net' || this.win.matchMedia('(display-mode: standalone)').matches || this.win.navigator.standalone === true;
  }
  ...
  async shareTradeOffer() {
    ...
    if (!offer) return;
    if (this.win.location.hostname === 'appassets.androidplatform.net') {
      this.win.location.href = `vexlcalc-share://share?text=${encodeURIComponent(offer)}`;
      return;
    }
    try {
  ...
  start() {
    ...
    elements.tradeShare.hidden = typeof win.navigator.share !== 'function' && win.location.hostname !== 'appassets.androidplatform.net';
    ...
    if (win.location.hostname !== 'appassets.androidplatform.net' && 'serviceWorker' in win.navigator && win.isSecureContext) win.navigator.serviceWorker.register('./sw.js').catch(() => {});
  }
}
