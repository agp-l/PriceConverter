...
const elements = {
  ...
  refresh:$('#refresh'), add:$('#add-currency'), dialog:$('#currency-dialog'),
...
function restore() {
  ...
  } catch { return {selected:['CZK','EUR','USD'],unit:'BTC',cache:null}; }
}

const stored = restore();
const state = {selected:stored.selected, unit:stored.unit, cache:stored.cache, anchor:'BTC', raw:'1', btc:1, busy:false, error:false};

function save() {
  try { localStorage.setItem(key, JSON.stringify({selected:state.selected,unit:state.unit,cache:state.cache})); } catch { /* Private mode or full storage: conversion still works. */ }
}

function format(value, code) {
  if (value === null || !Number.isFinite(value)) return '';
  const digits = code === 'BTC' ? (state.unit === 'SATS' ? 3 : 8) : 3;
  return new Intl.NumberFormat('cs-CZ', {maximumFractionDigits:digits, useGrouping:true}).format(value);
}
...
function renderRows() {
  ...
  if (!state.selected.length) {
    ...
    empty.className = 'empty-state'; empty.textContent = 'Zatím tu nejsou žádné měny. Přidejte si první.';
  ...
  for (const code of state.selected) {
    ...
    input.dataset.code = code; input.setAttribute('aria-label',`Částka v měně ${currency.name}`);
    input.addEventListener('input', () => recalculate(code,input.value));
    input.addEventListener('focus', () => input.select());
    value.append(input);
    const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'remove-button'; remove.textContent = '×'; remove.title = `Odebrat ${code}`; remove.setAttribute('aria-label',`Odebrat ${currency.name}`);
...
function setUnit(unit) {
  ...
  elements.caption.textContent = unit === 'SATS' ? 'Malé jednotky. Velké možnosti.' : 'Digitální peníze, bez hranic.';
...
function showStatus() {
  ...
  if (state.busy && !cache) { elements.status.textContent = 'Načítám kurzy…'; return; }
...
if (state.unit === 'SATS') { state.btc = 1/1e8; elements.btc.value = '1'; elements.unit.textContent = 'SATS'; elements.caption.textContent = 'Malé jednotky. Velké možnosti.'; elements.satsButton.classList.add('selected'); elements.satsButton.setAttribute('aria-pressed','true'); elements.btcButton.classList.remove('selected'); elements.btcButton.setAttribute('aria-pressed','false'); }
renderRows(); showStatus(); refresh();
