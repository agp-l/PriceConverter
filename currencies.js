...
export function getCurrencies(language = 'cs') {
  let names;
  try { names = new Intl.DisplayNames(localeFor(language), {type:'currency'}); } catch { /* Use codes as fallback. */ }
  return catalog.map(code => {
    const localized = names?.of(code);
    const name = (language === 'cs' && favorites[code]?.[0]) ||
      (localized && localized !== code ? localized[0].toLocaleUpperCase(localeFor(language)) + localized.slice(1) : code);
